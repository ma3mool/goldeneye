...
@torch.no_grad()
def gather_golden(goldeneye, data_iter, dataset, cuda_en=True, precision='FP32', verbose=False, debug=False):
    golden_data = {}
    ...
    for input_data in tqdm(data_iter):
        ...
        for img in range(len(images)):
            ...
            label=labels[img].item()
            if "PUG" in dataset:
                inf_label=convert_pred(inf_label, mapping)
                # if inf_label==999:
                #     continue
...
if __name__ == '__main__':
    ...
    if "IMAGENET" in getDataset():
        ...
        baseH = 224
        baseW = 224
    elif "FOOD101" in getDataset():
        baseH=224
        baseW=224
    elif "STL10" in getDataset():
        baseH=224
        baseW=224
    ...
    golden_data, good_imgs, bad_imgs, total_imgs = gather_golden(goldeneye_model, dataiter, getDataset(), \
            getBatchsize(), precision=getPrecision(), verbose=getVerbose(), debug=getDebug())
    # print(len(golden_data))
    # print(good_imgs, bad_imgs, total_imgs)
    # Golden data gathering
