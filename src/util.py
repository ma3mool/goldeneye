...
def check_args(args=None):
    ...
    parser.add_argument(
        "-n", "--dnn", help="Neural network", required="True", default="alexnet"
    )
    ...
    parser.add_argument(
        "-B", "--bitwidth", help="Bitwidth of number format", type=int, default=32
    )
    ...
    parser.add_argument(
        "-w", "--workers", help="Number of workers for dataloader", type=int, default=0
    )
    ...
    global batchsize_in, dnn_in, dataset_in, format_in, precision_in, output_in, cuda_in, bitwidth_in, radix_in, bias_in, injections_in, injectionsLoc_in, training_in, workers_in, quantize_in, verbose_in, debug_in
...
def getInjectionsLocation():
    return injectionsLoc_in
...
def load_dataset(
    DATASET, BATCH_SIZE, workers=0, training=False, shuffleIn=False, include_id=True
):
    if DATASET == "CIFAR10":
...
def load_file(file_name, compress=True):
    f = (
        bz2.BZ2File(file_name + ".p.bz2", "rb")
        if compress
        else open(file_name.strip(".p.bz2"), "rb")
    )
